...
namespace Kattis
{
    ...
    class PathTracing
    {
        ...
        {
            List<string> moves = new List<string>(); //To hold the input---each string is a move for Billy to walk
            ...
            while ((move = Console.ReadLine()) != null) //Problem description says input stops at the end of file character. loop putting input into the moves vector
            {
                moves.Add(move); //and shove it into the vector-- adds it to the end of the list
            }
            ...
            char[,] path = new char[1001, 1001]; //This holds the characters we will draw at the end. We will update characters in here as we traverse each move.
            path[row, col] = 'S'; //Mark Billy's starting location - 500, 500
            for (int i = 0; i < moves.Count; i++) //Loop for each move given in the input and stored in the moves vector
            {
